...
KevinNet دو release عمومی منتشر می‌کند. کدام برای شما مناسب است:

### 🟢 نسخه 4.1.6 — توصیه شده

نسخه فعلی. این چیزی است که باید دانلود کنید مگر اینکه دلیل خاصی برای
این کار نداشته باشید. تمام کارهایی که 3.2.2 انجام می‌داد را انجام
می‌دهد به اضافه:

- **theme روشن و تاریک** با یک‌کلیک toggle و تشخیص خودکار OS
- **دکمه اسکن DoH/DoT** که endpointهای رمزنگاری شده DNS را probe می‌کند
  (تشخیص آن برای DPI ایران بسیار سخت‌تر از UDP/53 معمولی است)
- نتایج اسکن DoH/DoT با همان دکمه Save که می‌شناسید مستقیماً در
  پروفایل‌های VayDNS ذخیره می‌شوند
- رابط بازطراحی شده با چیدمان کارت، نشان برند، نوار وضعیت مدرن، و
  تب فعال زیرخط‌دار
- منوی راست‌کلیک روی لیست resolver (کپی IP، کپی همه، باز کردن پوشه
  خروجی)
- نشانگر آخرین اجرا روی هر پروفایل (`۵ دقیقه پیش اجرا شد` و غیره)
- برنامه آخرین دامنه، پوشه خروجی و حالت VPN شما را بین launchها
  به یاد می‌آورد
- اعتبارسنجی ورودی با پیام‌های خطای واضح برای دامنه‌ها، کلیدها و
  نام پوشه‌های نامعتبر
- JSON پروفایل و فایل‌های config با مجوز 0600 روی Linux/macOS ساخته
  می‌شوند تا کاربران دیگر نتوانند کلیدهای رمزنگاری شما را بخوانند
- صفحه راهنمای HTML جدید themed در مرورگر شما باز می‌شود
- دکمه‌ها در حین اسکن غیرفعال می‌شوند تا از اختلال تصادفی جلوگیری شود
- چک‌سام SHA-256 در کنار هر release برای تأیید دانلود منتشر می‌شود
- مجموعه تست ۹۲ تستی pytest قبل از هر باینری در CI اجرا می‌شود

برای فهرست کامل تغییرات [CHANGELOG_FA.md](CHANGELOG_FA.md) را ببینید.

### 🟡 نسخه 3.2.2 — قدیمی

release پایدار قبلی. هنوز کار می‌کند، هنوز در صفحه Releases موجود
است. فقط در این موارد از این استفاده کنید:

- یک setup کاری روی 3.2.2 دارید و نمی‌خواهید چیزی را تغییر دهید
- یک regression در 4.1.6 مخصوص محیط خود گزارش داده‌اید و هنوز fix
  نشده است

برای بقیه، از **نسخه 4.1.6** استفاده کنید. فایل‌های پروفایل بین دو
نسخه سازگار هستند، پس می‌توانید بدون از دست دادن داده بین آن‌ها
رفت و برگشت کنید.
...
به [**صفحه Releases**](../../releases) بروید و یکی از دو نسخه را
انتخاب کنید:
...
همان باینری‌های پلتفرم در صفحه release نسخه 3.2.2 موجود هستند.
فقط در صورتی این‌ها را دانلود کنید که 4.1.6 برای شما کار نمی‌کند —
برای مقایسه [CHANGELOG_FA.md](CHANGELOG_FA.md) را ببینید.
