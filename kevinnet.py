...
# and it will be used instead of the embedded copy.

_EMBEDDED_PUBLIC_RESOLVERS = """
8.8.8.8
8.8.4.4
# Cloudflare
1.1.1.1
...
# CleanBrowsing
https://185.228.168.9/dns-query
https://doh.cleanbrowsing.org/doh/family-filter/

# CIRA Canadian Shield
https://149.112.121.10/dns-query
...
BUILTIN_RESOLVERS = _read_data_lines("public_resolvers.txt", _EMBEDDED_PUBLIC_RESOLVERS)

# ═══════════════════════════════════════════════════════════════
#  IRAN IPv4 CIDR RANGES  (loaded from data/iran_cidrs.txt)
# ═══════════════════════════════════════════════════════════════
IRAN_CIDRS_RAW = _read_data_lines("iran_cidrs.txt", _EMBEDDED_IRAN_CIDRS)
...
# ═══════════════════════════════════════════════════════════════
WHITE_DNS_UDP = _read_data_lines("white_dns_iran.txt", _EMBEDDED_WHITE_DNS)
...
def get_doh_endpoints() -> list[str]:
    ...
    text = _read_data_lines("doh_endpoints.txt", _EMBEDDED_DOH_ENDPOINTS)
...
def get_dot_endpoints() -> list[str]:
    ...
    text = _read_data_lines("dot_endpoints.txt", _EMBEDDED_DOT_ENDPOINTS)
    return [l.strip() for l in text.splitlines()
            if l.strip() and not l.strip().startswith("#")]
